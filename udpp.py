...
class VissimSimulation:
    ...
    def set_bus_flow(self, veh_input_id, volume):
        try:
            ...
            for i in range(1, time_intervals.Count + 1):
                ...
                old_volume = tiv.AttValue("Volume")

                if old_volume != volume:
                    print(f"[{self.name}] Bus volume changed: {old_volume} → {volume}")
    ...
    def set_vehicle_speed(self, vehicle_id, speed):
        try:
            ...
            old_speed = v.AttValue("Speed")

            if old_speed != speed:
                print(f"[{self.name}] Vehicle {vehicle_id} speed changed: {old_speed} → {speed}")
...
class SimulationManager:
    ...
    def set_active(self, idx):
        if 0 <= idx < 3:
            if self.active_index != idx:
                print(f"[Manager] Active simulation changed: sc{self.active_index} → sc{idx}")
    ...
    def apply_bus_to_all(self, veh_input_id, volume):
        print(f"[Manager] Applying bus volume {volume} to ALL simulations")
        for sim in self.simulations:
            sim.set_bus_flow(veh_input_id, volume)

    def set_speed_active(self, vehicle_id, speed):
        print(f"[Manager] Setting speed of vehicle {vehicle_id} to {speed} in active simulation")
